<?php
    ...
    if (isset($_GET["id"])){
        ...
        // Query
        $stmt = $con->prepare("SELECT `Sender`, `Message`, `Image` FROM conversas WHERE (Sender = ? AND Reciever = ?) OR (Reciever = ? AND Sender = ?) ORDER BY Id");
        $stmt->bind_param("iiii", $user_id, $uid, $user_id, $uid);
        $stmt->execute();
        $result = $stmt->get_result();
        $count = $result->num_rows;

        $getUser = $con->prepare("SELECT id, usuario, Picture FROM user WHERE (id LIKE ?) LIMIT 1");
        ...
        if ($count < 1) {
            ...
            $stmt = $con->prepare("UPDATE conversas1 SET `Unread` = 'n' WHERE (MainUser = ? AND OtherUser = ?)");
