<?php
    ...
    // Query
    $stmt = $con->prepare("SELECT * FROM conversas1 WHERE (MainUser = ?) ORDER BY Modification DESC");
    ...
    while ($inbox = $result->fetch_assoc()) {
        $stmt = $con->prepare("SELECT id, usuario, Picture FROM user WHERE (Id LIKE ?) LIMIT 1");
