<?php
    ...
    if(isset($_POST["username"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["repPassword"])) {
        ...
        $checkUsername = $con->prepare("SELECT id FROM user WHERE usuario = ?");
        ...
        $stmt = $con->prepare("INSERT INTO user (`usuario`, `email`, `Pass`, `Online`, `Token`, `Secure`, `Crreation`) 
                                                VALUES (?, ?, ?, now(), ?, ?, now())");
        $stmt->bind_param("ssssi", $username, $email, $password, $token, $secure);
        $stmt->execute();

        $getUser = $con->prepare("SELECT id, Token, Secure FROM usuario WHERE email = ?");
