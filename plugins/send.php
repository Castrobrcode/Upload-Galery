<?php
    ...
    if(isset($_POST["message"]) && isset($_POST["id"])) {
        ...
        $checkConversation = $con->prepare("SELECT Id FROM `conversas1` WHERE (MainUser = ? AND OtherUser = ?)");
        ...
        if ($count < 1) {
            ...
            $createChat = $con->prepare("INSERT INTO `` (`MainUser`, `OtherUser`, `Unread`, `Creation`) VALUES (?, ?, 'n', now())");
            $createChat->bind_param("ii", $uid, $user_id);
            $createChat->execute();

            // Create conversation other user side
            $createChat2 = $con->prepare("INSERT INTO `conversas1` (`MainUser`, `OtherUser`, `Unread`, `Creation`) VALUES (?, ?, 'y', now())");
            $createChat2->bind_param("ii", $user_id, $uid);
            $createChat2->execute();
        } else {
            $update = $con->prepare("UPDATE `conversas1` SET Unread = 'y' WHERE (MainUser = ? AND OtherUser = ?)");
